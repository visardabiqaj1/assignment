...
The script is defensive: it waits for elements to appear, avoids duplicates, uses MutationObserver to survive re-renders, injects CSS dynamically, and does not remove original functionality.

How to run
1. 	Open the Samsung UK cart page (/uk/cart/) after adding a Galaxy Z Fold7.
...
• 	Reusing DOM nodes: The holiday badge reuses the banner text and hides the original banner (display:none) so scripts that rely on it continue to work. The trade-in section is visually hidden via an accessible technique (.sg-visually-hidden) so the underlying functionality remains.
...
• 	Brand detection: Uses navigator.vendor and navigator.userAgent heuristics. If brand cannot be determined, it falls back to “Any Smartphone” or “your smartphone”.
...
• 	Performance: Uses a MutationObserver  with a small debounce to re-run idempotent initialization when the cart re-renders. Waiting timeouts are conservative to avoid long polling.
• 	Visual styling: CSS is injected dynamically and kept lightweight to match Samsung’s visual language (neutral background, Samsung-blue link color). You can tweak the CSS in the injectStyles function.
